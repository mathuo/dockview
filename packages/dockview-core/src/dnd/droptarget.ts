...
export interface DroptargetOptions {
    ...
    getOverrideTraget?: () => DropTargetTargetModel;
    className?: string;
...
export class Droptarget extends CompositeDisposable {
    ...
    constructor(
        ...
        this.dnd = new DragAndDropObserver(this.element, {
            onDragEnter: () => {
                this.options.getOverrideTraget?.()?.getElements();
            },
            onDragOver: (e) => {
                Droptarget.ACTUAL_TARGET = this;

                if (this._acceptedTargetZonesSet.size === 0) {
                    if (this.options.getOverrideTraget) {
                        return;
                    }
            ...
            onDragEnd: (e) => {
                if (
                    this.options.getOverrideTraget &&
                    Droptarget.ACTUAL_TARGET === this
                ) {
                    if (this._state) {
                ...
                this.options.getOverrideTraget?.().clear();
            },
            onDrop: (e) => {
                e.preventDefault();
    ...
    private toggleClasses(
        ...
        if (!this.options.getOverrideTraget && !this.overlayElement) {
            return;
        }
        ...
        if (this.options.getOverrideTraget) {
            const ta = this.options.getOverrideTraget?.().getElements();
            const el = ta.root;
            ...
            overlay.style.setProperty('--dv-overlay-height', `${box.height}`);

...
2. (_always_) Keep the DOM tree alive but hide it in order to allow the select panels content to show.
...
To keep an iframe alive, render its panel with `renderer: 'always'` so the element is never removed from the DOM (or set the dock's `defaultRenderer` to `'always'`):

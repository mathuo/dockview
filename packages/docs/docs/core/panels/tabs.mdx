...
When a group has more tabs than fit across the header, Dockview collapses the
surplus into an overflow dropdown by default, or can wrap the tabs onto multiple
rows so they all stay visible.

See [Tab Overflow & Multi-row Tabs](/docs/core/panels/tabOverflow) for the overflow
dropdown, the `overflow.mode: 'wrap'` multi-row layout, and cross-row drag reorder.

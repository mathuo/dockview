...
With the `autoEdgeGroups` option an edge takes up **zero space** when empty and is revealed by dragging a panel to it, like a VS Code sidebar — see [Drag-revealed edge groups](/docs/core/groups/autoEdgeGroups).

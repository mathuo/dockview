...
The `autoHideEdgeGroups` option sets the default for every edge group, but auto-hide is resolved **per group**: pass `autoHide` to `addEdgeGroup`, or call `api.setAutoHide` at runtime, to opt a single edge group in or out. This lets a static always-docked edge group and an auto-hiding one co-exist in the same layout.

Enable it with the `autoHideEdgeGroups` option — pass `true`, or an options object to tune the behaviour:
...
<DocRef declaration="AutoHideEdgeGroupOptions" />
...
- `openDelay` — milliseconds the pointer must dwell on the strip before the peek opens (keyboard focus opens immediately). Default `250`.
- `closeDelay` — milliseconds after leaving both the strip and the peek overlay before it closes; re-entering either cancels it. Default `300`.

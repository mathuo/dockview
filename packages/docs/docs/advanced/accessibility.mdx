...
- The tab strip is a `role="tablist"` (with `aria-orientation`), each tab a
  `role="tab"` carrying `aria-selected` and `aria-controls`.
...
- [Focus navigation](/docs/advanced/focus-navigation) — free `DockviewApi` methods for moving focus between panels and groups

...
[Migrating to v8](/docs/overview/migrating-to-v8).
...
separate row. Pinning round-trips through serialization and can be toggled from
the built-in tab context menu via the new `'pin'` menu item.
...
drag-and-drop reordering. See [Multi-row tabs](/docs/core/panels/multiRowTabs).
...
and centers snap to other floating groups, the container, and optionally the

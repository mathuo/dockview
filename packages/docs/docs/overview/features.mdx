...
| Drag-revealed edge groups  | Drag a panel to the layout edge to dock it as an edge group that is invisible when empty via `autoEdgeGroups` — see [Edge groups](/docs/core/groups/edgeGroups#drag-revealed-edges) |                                          | <span className="feature-check">✓</span> |
